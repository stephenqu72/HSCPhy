...
############################################
# 💼 Multi-user Auth + Per-user Storage
############################################
APP_TITLE = "HSC AI Tutoring Centre"
ACCOUNTS_DB = os.path.join("server", "users.json")  # server-side credentials store
USERS_ROOT = os.path.join("users")  # per-user project subfolders live here
...
You are a top HSC teacher. Based on the concepts shown in the input image, generate a high-quality Y12 NSW HSC-style question (not limited to multiple choice). Follow this structure:
{
  ...
  "Image_DataTable": "Python code to generate any diagram or data table, using Plotly or Matplotlib. Must define a function `generate_plot()`, by end of code, return fig.",
...
def extract_json_from_response_old(response_text):
...
def extract_json_from_response(response_text):
    """
    Extract and clean JSON from a model response that may be wrapped
    in code fences and contain invalid escape characters (e.g., \(a\)).
    Works with Ollama, Gemini, etc.
    """
    import codecs

    raw = getattr(response_text, "text", str(response_text)).strip()

    # 1. Remove ```json or ```python fences
    if raw.startswith("```"):
    ...
    if not m:
        raise ValueError(f"⚠️ No JSON object found.\nRaw content:\n{raw[:500]}")

    json_candidate = m.group(1)

    # 3. Fix invalid escape sequences like \(a\), \), etc.
    # Only allow legal JSON escapes: \" \\ \/ \b \f \n \r \t \uXXXX
    # Replace all other backslashes with double backslash \\ to make them valid

    def escape_invalid_backslashes(s):
        # Replace invalid backslashes with escaped ones
        def replacer(match):
            seq = match.group(0)
            if re.match(r'\\[\"\\/bfnrtu]', seq):
                return seq  # keep valid escapes
            return seq.replace("\\", "\\\\")  # escape invalid ones
        return re.sub(r'\\.', replacer, s)

    fixed = escape_invalid_backslashes(json_candidate)
...
def call_model(prompt, image):
    if selected_model == "Gemini":
        ...
        img_b64 = image_to_b64_png(image)
        try:
            r = requests.post(
            ...
            # Return a Gemini-like interface with .text for uniform handling
            return type("Resp", (), {"text": f"Error contacting Ollama at {OLLAMA_HOST}: {e}"})()

        if r.status_code != 200:
            return type("Resp", (), {"text": f"Ollama error ({r.status_code}): {r.text}"})()

        try:
            envelope = r.json()   # ✅ parse the outer JSON only
        except Exception as e:
            return type("Resp", (), {"text": f"Invalid JSON envelope from Ollama: {e}, content={r.text}"})()

        raw_text = envelope.get("response", "")
        return type("Resp", (), {"text": raw_text})()
...
def normalize_latex_markers(text: str) -> str:
    """
    Normalize math output from LLMs for proper rendering in Streamlit:
    - Fixes \\(...\\) → $...$, \\[...\\] → $$...$$
    - Fixes malformed math fences like $(...$), $(...$), etc.
    - Escapes invalid JSON-like sequences (\b, \g, etc.)
    - Unescapes \\alpha → \alpha for rendering
    """

    if not text:
        return ""

    # Step 1: Fix malformed fences like $( ... $)
    ...
    # Step 3: Escape invalid backslashes (e.g., \b, \g, \d)
    def escape_invalid_json_escapes(match):
        seq = match.group(0)
        # Allow: valid LaTeX commands like \alpha, \pi, etc.
        if re.match(r'\\[a-zA-Z]{2,}', seq):  # allow \alpha, \delta, etc.
            return seq
        # Allow: valid JSON escapes (like \n, \t, \\, etc.)
        if re.match(r'\\[\"\\/bfnrtu]', seq):
            return seq
        # Else: escape it
        return seq.replace("\\", "\\\\")

    text = re.sub(r'\\.', escape_invalid_json_escapes, text)

    # Step 4: Convert \\alpha → \alpha for LaTeX rendering
    text = text.replace("\\\\", "\\")

    return text


def render_markdown_field(fieldname: str, data: dict, title: str = None):
    """
    Extracts, normalizes, and renders a LaTeX-capable markdown field.
    - fieldname: key in the response (e.g., "Answer")
    - data: the model output JSON
    - title: optional header (e.g., "✅ Answer")
    """
    # Try variants of the fieldname (e.g., Detailed_Explanation vs. Detailed explanation)
    variants = [
        fieldname,
        fieldname.replace("_", " "),
        fieldname.replace("_", ""),
    ...
    for key in variants:
        if key in data:
            ...
            if raw:
                if title:
                    st.markdown(f"### {title}")
                st.markdown(normalize_latex_markers(raw))
                return
    st.info(f"No '{fieldname}' field found.")
...
############################################
st.sidebar.markdown("## 🔍 Select HSC Physics Module")
...
base_root = os.path.join("C:/Library", "HSCPhy", course_level)
...
selected_model = st.sidebar.selectbox("LLM Provider:", ["Gemini", "Ollama"], key="llm_choice")
...
past_paper_images = []
if mode == "Past Paper":
    list_file_path = f"{course_level}_List.txt"
    if not os.path.exists(list_file_path):
        st.error(f"❌ Past paper list file '{list_file_path}' not found in project folder.")
        st.stop()
    ...
    question_library = os.path.join("C:/Library", "HSCPhy", course_level)
    ...
    if not past_paper_images:
        st.warning(f"⚠️ No questions found for '{selected_paper}' in QuestionLibrary.")
        st.stop()
...
"""
                    ...
                    if ans:
                        st.markdown("#### ✅ Answer")
                        ans = normalize_latex_markers(ans)
                        st.markdown(ans)
                    else:
                    ...
                    if detail:
                        st.markdown("#### 📘 Details")
                        detail = normalize_latex_markers(detail)
                        st.markdown(detail)
